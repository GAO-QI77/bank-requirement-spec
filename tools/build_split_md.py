...
# -*- coding: utf-8 -*-
"""
生成交行内部模型可导入的拆分版 Markdown skills（6 文件版）。

银行内部平台单个 skill 有 6000 字硬上限；脚本按 5600 字做验证边界，
给平台解析和标题留余量。源文件仍按目录型 skill 维护，通过本脚本构建。

6 文件结构：
  00-router.md          总路由 + 核心规则（禁令/义务/占位符/原因代码）
  01-risk-routing.md    分级路由 + 条线 + 高敏感概念 + 样例 + 残余风险
  02-rounds-0-2.md      第 0/1/2 轮模板
  03-rounds-3a-4.md     第 3A/3B/4 轮模板
  04-third-party.md     第三方基础规则（机构/数据/系统/账务）
  05-risk-review.md     第三方差错/安全/参数/应急 + 模型自检与人工核验清单
"""
...
def normalize_core(text: str) -> str:
    ...
    replacements = {
        ...
        "> 配套要求：必须搭配《人工确认 Checklist V4.0》使用": "> 配套要求：必须执行 05-risk-review.md 的模型自检与人工核验要求",
        "# 第零部分　使用者必读（此部分不投喂模型）": "# 执行前约束",
        "## 0.4 调用编排（人工操作流程）": "## 0.4 分轮执行编排",
        "每轮模板均**继承第一部分模型规则**，由人工在调用时一并粘贴。": "每轮模板均**自动继承 00-router.md 的模型规则**。生成任一轮输出时，必须同时遵守硬禁令、硬义务、占位符和原因代码规则。",
    }
    for old, new in replacements.items():
        text = text.replace(old, new)
...
def normalize_third_party(text: str) -> str:
    ...
    replacements = {
        ...
        "★ 金额构成（本金/手续费/税费）涉及时，按 核心引擎章节 高敏感概念 D 类输出 Cxx 问题清单。借贷方向、科目、内部户、清算账户、手续费比例、清算周期一律禁止写死。": "★ 金额构成（本金/手续费/税费）涉及时，按 01-risk-routing.md 高敏感概念 D 类输出 Cxx 问题清单。借贷方向、科目、内部户、清算账户、手续费比例、清算周期一律禁止写死。",
...
def build_files() -> dict[str, str]:
    ...
    core_rules = slice_between(core, "# 执行前约束", "# 第二部分　分级路由")

    files = {
        # 00: Router + Core rules (merged for single-upload convenience)
        "00-router.md": (
            router()
            + "\n---\n\n"
            + core_rules
        ),
        # 01: Risk routing + sensitive concepts + examples + residual risk
        "01-risk-routing.md": (
            file_header(
                "01-risk-routing.md",
        ...
        # 04: Third-party foundation (only for third-party requirements)
        "04-third-party.md": (
            file_header(
                "04-third-party.md",
        ...
        # 05: Third-party risk + review checklist (merged; checklist needed by all tiers)
        "05-risk-review.md": (
            file_header(
                "05-risk-review.md",
                "第三方差错/安全/参数/应急/终稿专章 + 模型自检与人工核验清单。",
                "核验清单部分：每轮输出后均启用；第三方差错部分：第三方类第 3B/4 轮启用。",
                "04-third-party.md",
                "00-router.md",
            )
            + "\n"
...
def validate(files: dict[str, str]) -> None:
    ...
    if list(files) != expected:
        raise ValueError(f"拆分文件清单与预期不一致: {list(files)}")

    forbidden = [
        "SKILL.md",
        "references/",
        "core-engine.md",
        "references/third-party",   # 原始源文件路径引用；"04-third-party.md" 不触发
        "human-checklist.md",
        "不投喂模型",
        "Claude / Agent Skill",
    ]
...
def main() -> int:
    ...
    for filename, content in files.items():
        status = "✅" if len(content) <= MAX_CHARS else "❌"
        print(f"  {status}  {filename}: {len(content)} 字")
    return 0
